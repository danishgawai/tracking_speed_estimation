...
class BYTETracker(object):
    def __init__(
        ...
        self.buffer_size = int(self.frame_rate / self.reference_fps * self.track_buffer)
    ...
    def update(self, results):
        ...
        if len(results):
            if results.shape[1] == 6:
                ...
                scores = results[:, 4] #* output_results[:, 5]
                bboxes = results[:, :4]  # x1y1x2y2
        ...
        output_stracks = [[track.track_id, track.centroid, track.tlbr, track.score, int(track.cls)] for track in self.tracked_stracks if track.is_activated]
